...
export default function BlogsMedium({ username = "your-medium-username", showAll = false }: BlogsMediumProps) {
  ...
  return (
    <section id="blogs" className="py-20 bg-white/30">
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {displayPosts.map((post, index) => (
                ...
                >
                  <Card className="h-full group hover:shadow-xl transition-all duration-300 bg-white/90 backdrop-blur-sm border-white/70 relative">
                    {index === 0 && (
                    ...
                    <CardHeader className="p-0">
                      <div className="relative h-48 overflow-hidden rounded-t-lg bg-gray-100">
                        ...
                        <div className={`image-placeholder absolute inset-0 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center ${post.thumbnail ? 'hidden' : ''}`}>
                    ...
                    <CardContent className="p-6">
                      ...
                      <CardTitle className="text-xl mb-3 line-clamp-2 text-[#111111]">
                        {post.title}
                      </CardTitle>
                      {post.description && (
                    ...
                    <CardFooter className="p-6 pt-0">
                      <Button 
                        asChild 
                        variant="default" 
                        className="w-full group bg-gray-600 hover:bg-gray-700 text-white font-semibold shadow-md"
                      >
                        <a 
                          href={post.link} 
                          target="_blank" 
